// Trevor Cargile
// 813542789
// Project: Models with Mousetrack movement and colors.
// Original code by: Professor Jurgen Schulze
// x/X y/Y z/Z to rotate with keyboard.
...
/* Spot and Point variables to control disabling and enabling. */
int spot = 1;
int point = 1;
...
void Cube::objectloader(int key, int x, int y){
	if (key == GLUT_KEY_F1){
		//Reset color back to redish white.
		unique = 0;
		r = 0.4;
		g = 0.3;
		b = 0.3;
		...
		if (key == GLUT_KEY_F2){
			...
			//Change teddy color to green mix.
			cout << "Teddy model loading with no diffuse." << endl;
			r = 0.3;
			g = 0.6;
			b = 0.3;
			//Change material properties to have no diffuse.
			float diffuse[] = { 0.0, 0.0, 0.0, 0.0 };
			float specular[] = { 1.0, 1.0, 1.0, 1.0 };
			float ambience[] = { 0.2, 0.2, 0.2, 1.0 };
			glMaterialfv(GL_FRONT_AND_BACK, GL_DIFFUSE, diffuse);
			glMaterialfv(GL_FRONT_AND_BACK, GL_SPECULAR, specular);
			glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT, ambience);
		}
		else if (key == GLUT_KEY_F3){
			ObjReader::readObj("bunny.obj", nVerts, &vertices, &normals, nIndices, &indices);
			//Change bunny color to white.
			cout << "Bunny model loading with no specular." << endl;
			r = 0.5;
			g = 0.5;
			b = 0.5;
			//Change material properties to have no specular.
			...
			//Change dragon color to green blue mix.
			cout << "Dragon model loading with both diffuse and specular." << endl;
			r = 0.0;
			g = 0.3;
			b = 0.3;
			//Change material properties to have both diffuse and specular. CHANGE??
			float diffuse[] = { 1.0, 1.0, 1.0, 1.0 };
			float specular[] = { 1.0, 1.0, 1.0, 1.0 };
			float ambience[] = { 0.2, 0.2, 0.2, 1.0 };
			glMaterialfv(GL_FRONT_AND_BACK, GL_DIFFUSE, diffuse);
		...
		scalar = (sqrt(wnew * wnew + hnew * hnew) / 23) / d;
		cout << "Scale factor: " << scalar << endl;
	}
}

/* Used to rotate the objects with the keyboard, as well as toggle the lights. */
void Cube::processKeys(unsigned char key, int x, int y){
	//Create rotation matrix before hand.
	Matrix4 rotate;
	rotate.identity();
	switch (key){
		//Rotate around X counter clockwise.
		case 88:
			rotate.rotateX(0.1);
			matrix.multMM(rotate);
			break;
		...
		case 120:
			...
			matrix.multMM(rotate);
			break;
		//Rotate around Y counter clockwise.
		case 89:
			rotate.rotateY(-0.1);
			matrix.multMM(rotate);
			break;
		...
		case 121:
			...
			matrix.multMM(rotate);
			break;
		//Rotate around Z counter clockwise.
		case 90:
			rotate.rotateZ(-0.1);
			matrix.multMM(rotate);
			break;
...
/* Function called when the F1-F4 is pressed. */
...
int main(int argc, char *argv[])
{
  ...
  float light_position[] = { 6.0, 0.0, 10.0, 1.0 };
  float light_direction[] = { -1.0, 0.0, -1.0 };
  ...
  //Create pointlight:
  float light2_ambient[] = { 1.0, 0.0, 1.0, 1.0 };
  float light2_diffuse[] = { 1.0, 1.0, 1.0, 1.0 };
  float light2_specular[] = { 1.0, 1.0, 1.0, 1.0 };
  float light2_position[] = { -4.0, 0.0, 6.0, 1.0 };

...
void Fairy::draw(){
	...
	//Render the floor to test the lights.
	glMatrixMode(GL_MODELVIEW);
	glLoadMatrixd(fairy.getmatrix().getPointer());
	glBegin(GL_QUADS);
	glColor3f(1.0, 1.0, 1.0);		// Bottom will now appear blue/green mix.

	for (int i = 0; i < 100; i++){
		glNormal3f(0.0, 1.0, 0.0);
		glVertex3f(-10.0, -5.0, 9.5 - 0.5 * i);
		glVertex3f(10.0, -5.0, 9.5 - 0.5 * i);
		glVertex3f(10.0, -5.0, 10.0 - 0.5 * i);
		glVertex3f(-10.0, -5.0, 10.0 - 0.5 * i);
	}
	glEnd();
